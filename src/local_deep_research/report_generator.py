...
class IntegratedReportGenerator:
    ...
    def _build_no_boilerplate_directive(self) -> str:
        ...
        return (
            ...
            "   - '## 参考文献' / '## 参考资料' / '## 引用来源' / "
            "'## References' / '## Sources' (and any suffix: '## 参考文献"
            "标注说明', '## Sources Notes', '## References List', etc.) — "
            "the trailing ## Sources block is assembled by the report "
            "generator from the actual cited sources; do NOT write a "
            "per-section references list yourself, in any language or "
            "with any suffix.\n"
            ...
            "layout using symbols. This covers BOTH box-drawing "
            "(┌─┐│├└┘═║) AND arrows/connectors "
            "(→ ← ↑ ↓ ↔ ↕ ► ◄ — – ── →) used to draw route maps, "
            "distribution diagrams, flow charts, or topology. "
            "Concrete banned patterns: 'A → B → C' route lines, "
            "'node ──→ node' flow links, and multi-line layouts "
            "using ↓/↑ to connect columns. These render poorly "
            "across viewers and waste space. Describe routes as a "
            "numbered/ordered list; describe distributions, flows, "
            "or structure in prose or a table — never draw them "
            "with text characters or arrows.\n"
            "=== END OF OUTPUT RULES ===\n\n"
        )
