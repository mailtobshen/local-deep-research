...
def run_research_process(research_id, query, mode, **kwargs):
    ...
    try:
        ...
        try:
            ...
            # Hard abort on a dead proxy circuit / preflight done report:
            # proxy error is a POLICY abort that supersedes the
            # "preflight done" advisory message — emit the abort message
            # FIRST so the user sees the failure at the top of the
            # progress panel rather than after the "9/10 available"
        ...
        # 31c9bdb2). Deliberately OUTSIDE the preflight try/except:
        # that block's contract is "a broken probe must never abort
        # the research", which swallowed this ValueError in the first
        # version (13:30 run — preflight showed 电路不可用 yet the
        # research continued). Here the statuses have already been
        # collected and reported; the abort decision is a POLICY on
        # their content, not a probe failure. Only the explicit
        # circuit-level "Tor 电路不可用" detail aborts, and only when
        # darkweb is the primary engine — greeting-level errors and
        # degraded probes stay advisory.
        try:
            _primary_tool = (
            ...
            # statuses never got bound (preflight import failed before
